...
function StudentList() {
    ...
    return (
        <div className=' h-screen md:w-[75vw] lg:w-[85vw] pt-20 overflow-y-scroll relative'>
            <div className='w-[98%] bg-blue-300  mx-auto text-center  py-4 font-semibold text-xl'>Students</div>
            ...
            <div className='relative overflow-x-auto shadow-md sm:rounded-lg overflow-y-scroll text-center'>
                ...
                <table className='w-[98%] text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mx-auto mt-3 '>
                    <thead className='text-xs text-gray-700 uppercase bg-primary dark:text-gray-400 '><tr className=''><th className={`${deleteItem ? '' : 'hidden'}  block`}></th><th className='px-6 py-3'>name</th>{/*<th className='px-6 py-3'>Usn</th>*/}<th className='px-6 py-3'>mobile</th>{/*<th className='px-6 py-3'>Busno</th>*/}</tr></thead>
                    <tbody>
                        {
                            studentList ? (studentList.map((item, key) => (
                                <>
                                    <tr key={key} className='odd:bg-white even:bg-gray-50  hover:bg-gray-200'>
                                        {/* <Link className=''> */}
                                        <td className={`${deleteItem ? '' : 'hidden'}  py-1 px-4`}>
                                            <input
                                        ...
                                        <td scope="row" className="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white" onClick={() => { navigate(`/manageStudent?student=${item.userId}`) }}>
                                            <img className="w-14 p-1 h-14 rounded-full object-cover border-2 border-black/30" src={`/images/${item.userImage}`} alt="Jese image" />
                                            <div className="ps-3">
